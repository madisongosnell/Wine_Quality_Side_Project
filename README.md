...
These questions, among others, formed the foundation of this analysis. From the analysis, we see that pH of a wine will be influenced by the wine type, alcohol level, fixed acidity, sulphates, residual sugar, citric acid, total sulfur dioxide, and chlorides. This model explains roughly 37.3% of the variance of the pH variable. The quality of a wine will be influenced by the physiochemical properties of wine type, alcohol level, volatile acidity, sulphates, residual sugar, free sulfur dioxide, total sulfur dioxide, and chlorides. The ridge regression model for both pH and quality had the smallest mean absolute error, followed closely behind by the multiple linear regression models, then the lasso regression mdoel. 
